...
def get_accounts(plan, mnemonic, num_of_keys=21):
    PRE_FUNDED_ACCOUNTS = []
    plan.print("mnemonic: %s" % mnemonic)
    for current_key in range(num_of_keys):
        private_key_service_name = "run-ethereal-private-key-%s" % current_key
        eth_address_service_name = "run-ethereal-eth-address-%s" % current_key
        
        private_key = plan.run_sh(
            name=private_key_service_name,
            image=IMAGE,
            description="Running ethereal to derive private keys of key %s" % current_key,
            run="""private_key=$(/app/ethereal hd keys --seed="%s" --path="m/44'/60'/0'/0/%s" | awk '/Private key/{print substr($NF, 3)}'); echo -n $private_key""" % (
        ...
        eth_address = plan.run_sh(
            ...
            image=IMAGE,
            description="Running ethereal to derive eth address of key %s" % current_key,
            run="""eth_addr=$(/app/ethereal hd keys --seed="%s" --path="m/44'/60'/0'/0/%s" | awk '/Ethereum address/{print $NF}'); echo -n $eth_addr""" % (
                mnemonic, current_key
            ),
        )
