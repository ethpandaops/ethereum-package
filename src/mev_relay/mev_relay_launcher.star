...
DUMMY_BLS_SECRET_KEY = "0xae1c2ca7bbd6f415a5aa5bb4079caf0a5c273104be5fb5e40e2b5a2f080b2f5bd945336f2a9e8ba346299cb65b0f84c8"
...
def launch_mev_relay(plan, args, network_id):
    ...
    plan.add_service(
        ...
        config = ServiceConfig(
            ...
            cmd = ["housekeeper", "--network", network_name, "--db", "postgres://postgres:postgres@localhost:5432/postgres?sslmode=disable",]
        )
    )
    ...
    plan.add_service(
        ...
        config = ServiceConfig(
            image = MEV_BOOST_RELAY_IMAGE,
            cmd = ["api", "--network", network_name, "--db", "postgres://postgres:postgres@localhost:5432/postgres?sslmode=disable", "--secret-key", DUMMY_BLS_SECRET_KEY, "--listen-addr", "0.0.0.0:9062"],
            ports = {
    ...
    plan.add_service(
        ...
        config = ServiceConfig(
            image = MEV_BOOST_RELAY_IMAGE,
            cmd = ["website", "--network", network_name, "--db", "postgres://postgres:postgres@localhost:5432/postgres?sslmode=disable", "--listen-addr", "0.0.0.0:9060"],
            ports = {
