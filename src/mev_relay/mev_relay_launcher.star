...
def launch_mev_relay(plan, mev_params, network_id, beacon_uris, validator_root):
    ...
    plan.add_service(
        ...
        config = ServiceConfig(
            image = image,
            cmd = ["housekeeper", "--network", "custom", "--db", "postgres://postgres:postgres@postgres:5432/postgres?sslmode=disable", "--redis-uri", "redis:6379", "--beacon-uris", beacon_uris].extend(mev_params.mev_relay_housekeeper_extra_args),
            env_vars= env_vars
        )
    )
    ...
    api = plan.add_service(
        ...
        config = ServiceConfig(
            ...
            cmd = ["api", "--network", "custom", "--db", "postgres://postgres:postgres@postgres:5432/postgres?sslmode=disable", "--secret-key", DUMMY_SECRET_KEY, "--listen-addr", "0.0.0.0:{0}".format(MEV_RELAY_ENDPOINT_PORT), "--redis-uri", "redis:6379", "--beacon-uris", beacon_uris, "--blocksim", "http://el-2-geth-lighthouse:8545"].extend(mev_params.mev_relay_api_extra_args),
            ports = {
    ...
    plan.add_service(
        ...
        config = ServiceConfig(
            image = image,
            cmd = ["website", "--network", "custom", "--db", "postgres://postgres:postgres@postgres:5432/postgres?sslmode=disable", "--listen-addr", "0.0.0.0:{0}".format(MEV_RELAY_WEBSITE_PORT), "--redis-uri", "redis:6379", "https://{0}@{1}".format(DUMMY_PUB_KEY, MEV_RELAY_ENDPOINT)].extend(mev_params.mev_relay_website_extra_args),
            ports = {
