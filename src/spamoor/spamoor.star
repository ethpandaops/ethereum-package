...
def launch_spamoor(
    ...
    for index, spammer in enumerate(spamoor_params.spammers):
        if (
            "peerdas" in network_params.network or
    ...
    template_and_data_by_rel_dest_filepath[SPAMOOR_CONFIG_FILENAME] = config_template_and_data
    ...
    template_and_data_by_rel_dest_filepath[SPAMOOR_HOSTS_FILENAME] = hosts_template_and_data
...
def new_config_template_data(startup_spammer):
...
def new_hosts_template_data(participant_contexts, participant_configs):
    rpchosts = []
    for index, participant in enumerate(participant_contexts):
        (
            full_name,
        ...
        index_str = shared_utils.zfill_custom(
            index + 1,
            len(str(len(participant_contexts))),
        )
        rpchost = (
