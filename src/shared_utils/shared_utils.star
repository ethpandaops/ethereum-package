...
def get_network_name(network):
    ...
    if (
        network != constants.NETWORK_NAME.kurtosis and
        network != constants.NETWORK_NAME.ephemery and
        constants.NETWORK_NAME.shadowfork not in network and
...
def get_client_names(participant, index, participant_contexts, participant_configs):
    ...
    full_name = (
        ...
        "-{0}".format(vc_client.client_name) if vc_client != None and cl_client.client_name != vc_client.client_name else "{0}-{1}-{2}".format(
            index_str,
...
def get_cpu_mem_resource_limits(
    ...
    volume_size = (
        int(volume_size) if int(volume_size) > 0 else constants.VOLUME_SIZE[network_name][client_type + "_volume_size"]
    )
...
def process_extra_mounts(_, extra_mounts, extra_files_artifacts = {}):
    """
...
def get_tolerations(
        specific_container_tolerations = [],
