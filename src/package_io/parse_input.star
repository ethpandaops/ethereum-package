...
def parse_input(input_args):
    ...
    if required_num_validtors > actual_num_validators:
        fail(
            "required_num_validtors - {0} is greater than actual_num_validators - {1}".format(
                required_num_validtors, actual_num_validators
            )
        )
    ...
    if (
        ...
        and result["participants"][1]["el_client_type"] == NETHERMIND_NODE_NAME
    ):
        fail("nethermind can't be the first or second node")

    if result.get("mev_type") in ("mock", "full"):
        result = enrich_mev_extra_params(
            result, MEV_BOOST_SERVICE_NAME_PREFIX, FLASHBOTS_MEV_BOOST_PORT
        )
