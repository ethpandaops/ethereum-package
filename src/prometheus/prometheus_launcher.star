...
def get_metrics_jobs(
    ...
    for context in el_contexts:
        if len(context.el_metrics_info) >= 1 and context.el_metrics_info[0] != None:
            ...
            additional_config = execution_metrics_info[METRICS_INFO_ADDITIONAL_CONFIG_KEY]
            if additional_config != None:
                if additional_config.labels != None:
                    labels.update(additional_config.labels)
                if (
                    additional_config.scrape_interval != None and
                    additional_config.scrape_interval != ""
                ):
    ...
    for context in cl_contexts:
        if (
            len(context.cl_nodes_metrics_info) >= 1 and
            context.cl_nodes_metrics_info[0] != None
        ):
            ...
            metrics_jobs.append(
                new_metrics_job(
                    ...
                    labels = {
                        ...
                        "chain_id": "{0}".format(
                            network_params.network_id if network_params.network == constants.NETWORK_NAME.kurtosis else constants.NETWORK_ID[network_params.network],
                        ),
                    },
                ),
            )
