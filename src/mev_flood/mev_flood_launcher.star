...
def spam_in_background(plan, el_uri, mev_flood_extra_args):
    command = ["/bin/sh", "-c", "nohup ./run spam -r {0} -k {1} -u {2} -l deployment.json  --secondsPerBundle 7 >main.log 2>&1 &".format(el_uri, ADMIN_KEY, USER_KEY)]
    if mev_flood_extra_args:
        joined_extra_args = " ".join(mev_flood_extra_args)
        command = ["/bin/sh", "-c", "nohup ./run spam -r {0} -k {1} -u {2} -l deployment.json  --secondsPerBundle 7 {3} >main.log 2>&1 &".format(el_uri, ADMIN_KEY, USER_KEY, joined_extra_args)]
    plan.exec(
        service_name = "mev-flood",
        recipe = ExecRecipe(
            command = command
