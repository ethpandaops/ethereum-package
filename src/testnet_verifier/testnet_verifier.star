...
def launch_testnet_verifier(plan, params, el_client_contexts, cl_client_contexts):
    config = get_asynchronous_verification_config(
        params, el_client_contexts, cl_client_contexts
    )
    plan.add_service(SERVICE_NAME, config)


def run_synchronous_testnet_verification(
    plan, params, el_client_contexts, cl_client_contexts
):
    ...
    exec_result = plan.exec(
        recipe=ExecRecipe(command=command), service_name=SERVICE_NAME
    )
    if exec_result["code"] != 0:
        error('Expected exec_result["code"] to be equal to 0')
...
def get_asynchronous_verification_config(
    params, el_client_contexts, cl_client_contexts
):
