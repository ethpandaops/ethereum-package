...
def get_config(
    ...
    if port_publisher.el_enabled:
        ...
        if (
            ...
            additional_public_port_assignments[constants.RBUILDER_PORT_ID] = public_ports_for_component[5]
            additional_public_port_assignments[constants.RBUILDER_METRICS_PORT_ID] = public_ports_for_component[6]
        public_ports.update(
    ...
    if (
        ...
        used_port_assignments[constants.RBUILDER_METRICS_PORT_ID] = RBUILDER_METRICS_PORT_NUM
    ...
    if persistent:
        ...
        files[EXECUTION_DATA_DIRPATH_ON_CLIENT_CONTAINER] = Directory(
            ...
            size = int(participant.el_volume_size) if int(participant.el_volume_size) > 0 else constants.VOLUME_SIZE[volume_size_key][constants.EL_TYPE.reth + "_volume_size"],
        )
...
def get_el_context(plan, service_name, service, launcher):
