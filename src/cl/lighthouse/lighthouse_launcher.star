...
def get_beacon_config(
    ...
    discovery_port_quic = (
        public_ports_for_component[3] if public_ports_for_component else BEACON_QUIC_PORT_NUM
    )
    ...
    if persistent:
        ...
        files[BEACON_DATA_DIRPATH_ON_BEACON_SERVICE_CONTAINER] = Directory(
            ...
            size = int(participant.cl_volume_size) if int(participant.cl_volume_size) > 0 else constants.VOLUME_SIZE[volume_size_key][constants.CL_TYPE.lighthouse + "_volume_size"],
        )
