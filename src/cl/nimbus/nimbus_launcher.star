...
def get_beacon_config(
    ...
    nimbus_checkpoint_sync_subtask_str = "{0} trustedNodeSync --data-dir={1} --trusted-node-url={2} --network={3} --backfill=false".format(
        ...
        checkpoint_sync_url,
        network_params.network if network_params.network in constants.PUBLIC_NETWORKS else constants.GENESIS_CONFIG_MOUNT_PATH_ON_CONTAINER,
    )
    ...
    if node_keystore_files != None and not participant.use_separate_vc:
        ...
        files[VALIDATOR_KEYS_MOUNTPOINT_ON_CLIENTS] = node_keystore_files.files_artifact_uuid
    ...
    if persistent:
        ...
        files[BEACON_DATA_DIRPATH_ON_SERVICE_CONTAINER] = Directory(
            ...
            size = int(participant.cl_volume_size) if int(participant.cl_volume_size) > 0 else constants.VOLUME_SIZE[volume_size_key][constants.CL_TYPE.nimbus + "_volume_size"],
        )
