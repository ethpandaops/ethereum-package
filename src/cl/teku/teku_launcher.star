...
def get_beacon_config(
    ...
    cmd = [
        ...
        "--network={0}".format(
            network_params.network if network_params.network in constants.PUBLIC_NETWORKS else constants.GENESIS_CONFIG_MOUNT_PATH_ON_CONTAINER + "/config.yaml",
        ),
    ...
    if persistent:
        ...
        files[BEACON_DATA_DIRPATH_ON_SERVICE_CONTAINER] = Directory(
            ...
            size = int(participant.cl_volume_size) if int(participant.cl_volume_size) > 0 else constants.VOLUME_SIZE[volume_size_key][constants.CL_TYPE.teku + "_volume_size"],
        )
