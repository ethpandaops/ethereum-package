...
def get_beacon_config(
    ...
    discovery_port_tcp = (
        public_ports_for_component[0] if public_ports_for_component else DISCOVERY_TCP_PORT_NUM
    )
    discovery_port_udp = (
        public_ports_for_component[1] if public_ports_for_component else DISCOVERY_UDP_PORT_NUM
    )
    discovery_port_quic = (
        public_ports_for_component[0] if public_ports_for_component else DISCOVERY_QUIC_PORT_NUM
    ...
    if persistent:
        ...
        files[BEACON_DATA_DIRPATH_ON_SERVICE_CONTAINER] = Directory(
            ...
            size = int(participant.cl_volume_size) if int(participant.cl_volume_size) > 0 else constants.VOLUME_SIZE[volume_size_key][constants.CL_TYPE.prysm + "_volume_size"],
        )
...
def new_prysm_launcher(el_cl_genesis_data, jwt_file):
    return struct(
