...
testnet_verifier = import_module(
    "github.com/kurtosis-tech/eth2-package/src/testnet_verifier/testnet_verifier.star"
)
...
def run(plan, args):
    ...
    all_participants, cl_genesis_timestamp = eth_network_module.run(
        plan, args_with_defaults_dict
    )
    ...
    # otherwise dummy relays spinup if chosen
        ...
        endpoint = mock_mev_launcher_module.launch_mock_mev(
            plan, el_uri, beacon_uri, jwt_secret
        )
    ...
    transaction_spammer.launch_transaction_spammer(
        plan, genesis_constants.PRE_FUNDED_ACCOUNTS, all_el_client_contexts[0]
    )
