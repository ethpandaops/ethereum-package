...
def run(plan, args = {}):
    ...
    for index, participant in enumerate(args_with_right_defaults.participants):
        if (
            num_participants == 1 and
            participant.cl_type == constants.CL_TYPE.lighthouse
        ):
            if (
                "--target-peers=0" not in participant.cl_extra_params and
    ...
    prometheus_private_url = None
    prometheus_grafana_index = None
    ...
    if network_params.perfect_peerdas_enabled:
        ...
        for index, participant in enumerate(args_with_right_defaults.participants[:16]):
            if participant.cl_type == constants.CL_TYPE.lodestar:
                ...
                participant.cl_type == constants.CL_TYPE.lighthouse or
                participant.cl_type == constants.CL_TYPE.grandine
            ):
                raw_node_key = (
                    static_files.PEERDAS_NODE_KEY_FILEPATH +
                    participant.cl_type +
    ...
    if "tempo" in args_with_right_defaults.additional_services:
        tempo_otlp_grpc_url = "http://{}:{}".format(
            tempo.SERVICE_NAME,
            tempo.OTLP_GRPC_PORT_NUMBER,
        )
        tempo_query_url = "http://{}:{}".format(
            tempo.SERVICE_NAME,
            tempo.HTTP_PORT_NUMBER,
        )
    ...
    first_participant = args_with_right_defaults.participants[0]
    if (
        hasattr(first_participant, "builder_network_params") and
        first_participant.builder_network_params != None
    ):
        mev_endpoints = first_participant.builder_network_params.relay_end_points
